...
for sSize in tqdm((swarmSize), desc= "Swarm size"):
    for a in tqdm(alpha, desc= "Alpha values"):
        for b in tqdm(beta, desc= "Beta values"):
            for rh in tqdm(rho, desc= "Rho values"):
...
with open('ACOparamRecomender.txt', 'w') as f:
    for item in fitnesses:
        f.write(item)
    f.write("Recomended parameters for ACO: " + saveFitMax)