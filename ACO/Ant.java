...
public class Ant extends Thread {
    ...
    public double getObjectiveValue() {
        ...
        if (objectiveValue == 0.0) {
            ...
            objectiveValue = Configuration.INSTANCE.distanceObjectivePunishment * distance
                    + Configuration.INSTANCE.timeWindowObjectivePunishment * timePenalty;
        }
    ...
    /*
     * Lay pheromone on the path
     */
    public void layPheromone(int id) {
        double pheromone = Configuration.INSTANCE.Q / objectiveValue;

        if (Configuration.INSTANCE.isDebug) {
            Configuration.INSTANCE.logEngine.write("--- Ant.layPheromone()  " + "Ant " + id);
    ...
    public void lookForWay() {
        // loop over trucks
        if (Configuration.INSTANCE.isDebug) {
            Configuration.INSTANCE.logEngine
                    .write("Ant: " + Thread.currentThread().getName() + " is looking for a way");
        }
        for (int k = 0; k < Configuration.INSTANCE.numberOfTrucks; k++) {
            ...
            for (int i = 0; i < Configuration.INSTANCE.assignToTruck; i++) {
                ...
                for (int j = 0; j < notYetVisited.size(); j++) {
                    double tau = Math.pow(antColony.getPheromone(route.get(i), notYetVisited.get(j)),
                            Configuration.INSTANCE.alpha);
                    double distance = Configuration.INSTANCE.distanceEtaPunishment
                            * data.getDistance(route.get(i), notYetVisited.get(j));
                    double eta = Math.pow((1 / distance), Configuration.INSTANCE.beta);

                    if (time > data.getCustomer(notYetVisited.get(j)).getDueDate()) {
                        double dueDelta = Configuration.INSTANCE.timeWindowEtaPunishment
                                * Math.abs(data.getCustomer(notYetVisited.get(j)).getDueDate() - time);
                ...
                Configuration.INSTANCE.merceneLock.lock();
                double random = Configuration.INSTANCE.randomGenerator.nextDouble();
                Configuration.INSTANCE.merceneLock.unlock();
                double cumulation = 0;
                for (int j = 0; j < notYetVisited.size(); j++) {
                    ...
                    if (data.getCustomer(notYetVisited.get(j)).getDemand()
                            + capacity > Configuration.INSTANCE.capacity) {
                    ...
                    if (random < selectionProbability + cumulation) {
                        ...
                        capacity += data.getCustomer(notYetVisited.get(j)).getDemand();
    ...
    public void run() {
        if (Configuration.INSTANCE.isDebug) {
            Configuration.INSTANCE.logEngine
                    .write("Ant: " + Thread.currentThread().getName() + " is starting new round");
        }
        newRound();
        if (Configuration.INSTANCE.isDebug) {
            Configuration.INSTANCE.logEngine
        ...
        lookForWay();
        if (Configuration.INSTANCE.isDebug) {
            Configuration.INSTANCE.logEngine.write("Ant: " + Thread.currentThread().getName() + " done with round");
        }

    }

    /*
     * Function that checks if route has all the customers and no duplicates
     */
    ...
    /*
     * Function that calculates the overall time a route is overdue by
     */
    ...
    /*
     * Function that calculates the overall length of a route
     */
